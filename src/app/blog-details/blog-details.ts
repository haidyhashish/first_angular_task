...
export class BlogDetails {
  intro: string = '';
  sections: { intro: string; title: string; paragraphs: string[] }[] = [];
  post: any;
  recommended: Post[] = [];
    ...
    if (this.posts[i].slug === slug) {
      ...
      if (this.post) {
        ...
        for (let i = 1; i < parts.length; i++) {
          ...
          const allLines = text.slice(firstNewLine + 1).split('\n');
      ...
      if (this.post) {
        for (let i = 0; i < this.posts.length; i++) {
          if (
            this.posts[i].category === this.post.category &&
            this.posts[i].id !== this.post.id
          ) {
            this.recommended.push(this.posts[i]);
          }

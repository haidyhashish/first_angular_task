...
export class Blog {
  ...
  totalPages(): number {
    ...
    let total = Math.ceil(this.totalPosts() / this.numOfPosts);
    return total;
  }

  listNumOfPages(): number[] {
    let list: number[] = [];
  ...
  postsInPage(): Post[] {
    ...
    let start = (this.currentPage - 1) * this.numOfPosts;
    for (let i = start; i < start + this.numOfPosts && i < posts.length; i++) {
      result.push(posts[i]);
    }
    return result;
  }

  movePages(page: number): void {
  ...
  reset(): void {
    ...
    this.searchTerm = '';
    this.currentPage = 1;
    const input = document.querySelector<HTMLInputElement>('input[placeholder="ابحث في المقالات..."]');
    if (input) input.value = '';
  }
  constructor(route: ActivatedRoute) {
    this.category = route.snapshot.paramMap.get('category') || 'الكل';
    this.currentPage = 1;
  }
